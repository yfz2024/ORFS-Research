...
class OptimizationWorkflow:
    ...
    def _build_error_context(self, data: Dict[str, Any]) -> str:
        """Summarize errors across runs so the LLM can diagnose failures"""
        log_data = data.get('log_data') or {}
        runs = log_data.get('runs') or []
        if not runs:
            return "No run history available for diagnostics yet."

        summary = log_data.get('summary') or {}
        total_runs = summary.get('total_runs', len(runs))
        failed_runs = summary.get('failed_runs', sum(1 for run in runs if not run.get('success', True)))
        successful_runs = summary.get('successful_runs', total_runs - failed_runs)
        runs_with_errors = sum(1 for run in runs if run.get('errors'))

        error_buckets: Dict[str, Dict[str, Any]] = {}
        failure_examples = []

        for run in runs:
            run_label = run.get('run_id') or run.get('file') or (run.get('files') or ["unknown"])[0]
            run_files = [os.path.basename(f) for f in run.get('files', [])]
            run_errors = [" ".join(err.split()) for err in run.get('errors', [])]

            if not run.get('success', True) or run_errors:
                failure_examples.append({
                    ...
                    'metrics': {
                        ...
                        for key in ['worst_slack', 'tns', 'cts_ws', 'total_wirelength']
                        if key in run.get('metrics', {})
                    }
                })

            for err in run_errors:
                bucket = error_buckets.setdefault(err, {'count': 0, 'runs': set(), 'files': set()})
                bucket['count'] += 1
                bucket['runs'].add(run_label)
                bucket['files'].update(run_files)

        summary_lines = [
            f"Total runs analyzed: {total_runs}",
            f"Successful: {successful_runs}, Failed: {failed_runs}, Runs with explicit errors: {runs_with_errors}"
        ]

        if error_buckets:
            summary_lines.append("Top recurring error signatures (max 5):")
            sorted_errors = sorted(error_buckets.items(), key=lambda kv: kv[1]['count'], reverse=True)
            for idx, (msg, info) in enumerate(sorted_errors[:5], start=1):
                runs_list = ", ".join(sorted(info['runs'])) or "unknown"
                files_list = ", ".join(sorted(info['files'])) or "unknown"
                summary_lines.append(
                    f"  {idx}. '{msg}' | count={info['count']} | runs={runs_list} | files={files_list}"
                )
        else:
            summary_lines.append("No explicit error strings were found in the available logs.")

        if failure_examples:
            summary_lines.append("Sample failing runs with metric snapshots:")
            for example in failure_examples[:3]:
                file_info = ", ".join(example['files']) if example['files'] else "unknown file"
                error_info = "; ".join(example['errors']) if example['errors'] else "No explicit errors logged"
                metric_info = ", ".join(f"{k}={v}" for k, v in example['metrics'].items()) or "metrics unavailable"
                summary_lines.append(
                    f"  - Run {example['run']} ({file_info}): errors={error_info}; metrics={metric_info}"
                )
    ...
    def _generate_llm_prompt(self, stage: str, data: Dict[str, Any]) -> str:
        ...
        error_context = self._build_error_context(data)
        error_prompt_prefix = (
            "Error and failure diagnostics extracted from logs:\n"
            f"{error_context}\n\n"
            "Use the above diagnostics to reason about likely failure modes, "
            "root causes, and mitigation strategies while answering the following tasks.\n\n"
        )
        ...
        if stage == 'inspect':
            prompt = (
                ...
                "5. A prioritized error diagnosis report: group recurring errors, map them to probable flow stages, "
                "highlight missing metrics/log anomalies, and propose concrete debugging or mitigation actions.\n"
            )
        elif stage == 'model':
            prompt = (
                f"**Stage: Model**\n"
                ...
                "5. Ways the model can help detect or avoid the diagnosed failure modes (e.g., additional features, "
                "constraints, or uncertainty thresholds that flag risky configurations).\n"
            )
    ...
    def _call_llm(self, stage: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"[DEBUG] Context message for {stage}:\n{context_message}\n")
        ...
        for block in response.content:
            if block.type == 'tool_use':
                ...
                # Capture any free-form reasoning so users can read root-cause analysis
                note = getattr(block, 'text', '')
                if note:
                    narrative_notes.append(note.strip())

        if narrative_notes:
            print("\nLLM diagnostic notes (non-tool response):")
            for idx, note in enumerate(narrative_notes, start=1):
                print(f"{idx}. {note}")
            ...
            for log_file in files:
                if log_file.endswith(('.log', '.json')):
                    ...
                    if log_file.startswith(pattern) or (self.platform in root and self.design in root):
                        ...
                        run_id = "main"  
                        # case1：eg. logs/asap7_aes_run1.log
                        m = re.search(r'_run(\d+)\.log$', log_file)
                        if m:
                            run_id = f"base_{m.group(1)}"
                        # case2：path includes base_*
                        elif 'base_' in root:
