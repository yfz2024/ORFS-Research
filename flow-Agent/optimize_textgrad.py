...
def process_log_file(log_path: str) -> Dict[str, Any]:
    ...
    with open(log_path, 'r') as f:
        ...
        # if "run3" in log_path:  # 👈 你可以换成任意 run，比如 run1 或 run2
...
class ReActFramework:
    ...
    def __init__(self, client, model_name="DeepSeek-R1"):
        self.client = client
    ...
    def __init__(self, platform: str, design: str, objective: str):
        ...
        self.react_framework = ReActFramework(
            client=OpenAI(
                ...
                # api_key="X7NJOWYHJX7A1BDACXRLWC1ZMCUWM4FX0Q6FHJ1Q",
                api_key="MZ1RSHGF7QPGC8HHBOSPBQTDAZ6QD9NBCHROSZ1B",
            ),
        ...
        self.supervisor_client = OpenAI(
            base_url=os.environ.get("SUPERVISOR_BASE_URL", "https://ai.gitee.com/v1"),
            api_key=os.environ.get("SUPERVISOR_API_KEY", "MZ1RSHGF7QPGC8HHBOSPBQTDAZ6QD9NBCHROSZ1B"),
        )
...
"""
    ...
    **Available Data**:
    ...
    def run_iteration(self, num_runs: int) -> Dict[str, Any]:
        """Run a single inspect-optimize-agglomerate flow and return its summary."""
        return self._execute_inspect_optimize_flow(num_runs, flow_index=0)

    # def run_dual_supervised_iterations(self, num_runs: int, num_flows: int) -> None:
    #     """
    #     Run multiple inspect-optimize-agglomerate flows with supervisor feedback injected
    #     into the next flow's prompt.
    #     """
    #     if num_flows <= 0:
    #         print("[WARN] num_flows must be >= 1, skipping execution.")
    #         return

    #     self.supervision_feedback = ""
    #     for flow_index in range(num_flows):
    #         print(f"\n=== Dual-Supervised Flow {flow_index + 1}/{num_flows} ===")
    #         flow_summary = self._execute_inspect_optimize_flow(num_runs, flow_index=flow_index)
    #         supervisor_feedback = self._run_supervisor_review(flow_summary, flow_index)
    #         if supervisor_feedback:
    #             self.supervision_feedback = supervisor_feedback
    #             print("[SUPERVISOR] Stored feedback for the next flow iteration.")
    #         else:
    #             print("[SUPERVISOR] Supervisor produced no feedback; continuing without updates.")
    ...
    def run_dual_supervised_iterations(self, num_runs: int, num_flows: int) -> None:
        ...
        for flow_index in range(num_flows):
            ...
            # Run Flow (this will use the latest version from self.tool_instructions).
            flow_summary = self._execute_inspect_optimize_flow(num_runs, flow_index=flow_index)
            
            # Get gradient (Supervisor Feedback)
            supervisor_feedback = self._run_supervisor_review(flow_summary, flow_index)
            
            if supervisor_feedback and flow_index < num_flows - 1:
    ...
    def _execute_inspect_optimize_flow(self, num_runs: int, flow_index: int = 0) -> Dict[str, Any]:
        """Internal helper containing the original inspect-optimize-agglomerate flow."""
        print(f"\n=== Starting optimization iteration #{flow_index + 1} for {self.platform}/{self.design} ===")
    ...
    def _run_supervisor_review(self, flow_summary: Dict[str, Any], flow_index: int) -> str:
        """
...
def main():
    if len(sys.argv) not in (5, 6):
        print("Usage: optimize_dual.py <platform> <design> <objective> <num_runs> [<num_flows>]")
        sys.exit(1)
    ...
    num_flows = int(sys.argv[5]) if len(sys.argv) == 6 else 1
    
    workflow = OptimizationWorkflow(platform, design, objective)
    # Keep backward compatibility: a single flow behaves like the legacy run_iteration.
